...
class SslCommerzNotification extends AbstractSslCommerz
{
    ...
    public function __construct()
    {
        ...
        $apiCredentials = config('sslcommerz.apiCredentials');

        $this->setStoreId($apiCredentials['store_id']);
        $this->setStorePassword($apiCredentials['store_password']);
    }
    ...
    public function makePayment(array $requestData, $type = 'checkout', $pattern = 'json')
    {
        ...
        if ($type == 'hosted') {
            if (!empty($formattedResponse['GatewayPageURL'])) {
                ...
                if (strpos($formattedResponse['failedreason'], 'Store Credential') === true) {
