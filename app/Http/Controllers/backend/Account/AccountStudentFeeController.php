...
class AccountStudentFeeController extends Controller
{
    ...
    public function AccountStudentFeeGetStudent(Request $request)
    {
        ...
        foreach ($data as $key => $std) {
            $registrationfee = StudentFeeCategoryAmount::where('fee_category_id', $fee_category_id)->where('class_id', $std->class_id)->first();
            ...
            $html[$key]['tdsource'] .= '<td>' . $registrationfee->fee_category_amount . '$' . '<input type="hidden" name="date" value= " ' . $date . ' " >' . '</td>';

            $html[$key]['tdsource'] .= '<td>' . $std['discount']['discount'] . '%' . '</td>';

            $orginalfee = $registrationfee->fee_category_amount;
    ...
    public function AccountStudentFeeStore(Request $request){
        $date = date('Y-m',strtotime($request->date));
        ...
        if (!empty(@$data) || empty($checkdata)) {
            ...
            return redirect()->route('account.student.fee.view')->with($notification);
        }else{
